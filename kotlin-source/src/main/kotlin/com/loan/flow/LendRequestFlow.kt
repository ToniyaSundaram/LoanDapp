 ...
 */
object LendRequestFlow {
    @InitiatingFlow
    @StartableByRPC
    class Lender(   val loanAmount: Int,
        ...
        override fun call(): SignedTransaction {
           ...
           // Changing the unconsumed state to consumed state.
            val expression = builder { LoanSchemaV1.PersistentLoan::status.equal("Active") }
           ...
           val loanState = LoanState(loanAmount, serviceHub.myInfo.legalIdentities.first(),interestRate, otherParty, status)
            ...
            val txCommand = Command(LoanContract.Commands.Lend(), loanState.participants.map { it.owningKey })
            ...
            val fullySignedTx = subFlow(CollectSignaturesFlow(partSignedTx, setOf(otherPartyFlow1,otherPartyFlow2), LoanRequestFlow.Initiator.Companion.GATHERING_SIGS.childProgressTracker()))
        ...
        override fun call(): SignedTransaction {
            val signTransactionFlow = object : SignTransactionFlow(otherPartyFlow) {
                override fun checkTransaction(stx: SignedTransaction) = requireThat {
                    ...
                    "This must be an Lend transaction." using (output is LoanState)
